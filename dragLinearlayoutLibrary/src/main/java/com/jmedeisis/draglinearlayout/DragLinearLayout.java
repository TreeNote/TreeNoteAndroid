...
public class DragLinearLayout extends LinearLayout {
    ...
    public interface OnViewSwapListener {
        /**
         * Invoked right after the items are swapped due to a drag event.
         */
        void onSwap(View draggedView, View newAboveView, View newBelowView);
    }
    ...
    private class DragItem {
        ...
        // View, auf über welche gedragged wird.
        private View draggedOverView;
        ...
        public boolean hasDraggedOverView() {
            return draggedOverView != null;
        }

        public void onDragStart() {
            if (dragAndDropListener != null) {
                dragAndDropListener.onDragStart(view);
            }
        ...
        public void onDragStop() {
            this.dragging = false;
            if (dragAndDropListener != null) {
                View droppedOnView = null;
                if (draggedOverView != null) {
                    droppedOnView = draggedOverView;
                    draggedOverView = null;
                }

                int newAbovePosition = previousDraggablePosition(draggedItem.position);
                int newBelowPosition = nextDraggablePosition(draggedItem.position);
                View newAboveView = getChildAt(newAbovePosition);
                View newBelowView = getChildAt(newBelowPosition);

                dragAndDropListener.onDrop(view, droppedOnView, newAboveView, newBelowView);
            }
        }
        ...
        public void redrawDraggedDrawable() {

            view.post(new Runnable() {
                @Override
                public void run() {
                    Rect oldBounds = viewDrawable.getBounds();
                    viewDrawable = getDragDrawable(view);
                    viewDrawable.setBounds(oldBounds);
                }
            });
        }

        public void setDraggedOverView(View draggedOverView) {
            this.draggedOverView = draggedOverView;
            if (dragAndDropListener != null) {
                dragAndDropListener.onDragOverEnter(view, draggedOverView);
                draggedItem.redrawDraggedDrawable();
            }
        }

        public void removeDraggedOverView() {
            if (draggedOverView != null) {
                if (dragAndDropListener != null) {
                    ...
                    dragAndDropListener.onDragOverLeave(view, draggedOverView, newAboveView, newBelowView);
                    draggedItem.redrawDraggedDrawable();
                }
                draggedOverView = null;
            }
...
//            dragHandle.setOnLongClickListener(new DragHandleOnTouchListener(child));
    ...
    /**
     * See {@link com.jmedeisis.draglinearlayout.DragAndDropListener}.
     */
    ...
    private void onDrag(final int offset) {
        ...
        // befindet sich die gedraggte View im unteren Drittel der unteren View
        final boolean isBelow = (belowView != null) &&
                (currentTop + draggedItem.height > belowView.getTop() + belowView.getHeight() / 3);

        // befindet sich die gedraggte View im oberen Drittel der oberen View
        final boolean isAbove = (aboveView != null) &&
                (currentTop < aboveView.getTop() + aboveView.getHeight() / 3);

        if (!isAbove && !isBelow) {
            // befindet sich die gedraggte View im mittleren Drittel einer View
            Optional<View> draggedOverViewOptional = checkIsInnerView(currentTop, aboveView, belowView);

            if (draggedOverViewOptional.isPresent()) {
                if (!draggedItem.hasDraggedOverView()) {
                    draggedItem.setDraggedOverView(draggedOverViewOptional.get());
                }
            } else {
                if (draggedItem.hasDraggedOverView()) {
                    draggedItem.removeDraggedOverView();
                }
            }
        }
        ...
        if (isBelow || isAbove) {
            ...
            if (null != swapListener) {
                ...
                swapListener.onSwap(draggedItem.view, newAboveView, newBelowView);
                // Damit die draggedItem.viewDrawable an die richtige, aktuelle View angepasst wird
                draggedItem.redrawDraggedDrawable();
            }
            ...
            switchViewObserver.addOnPreDrawListener(new OnPreDrawListener() {
                ...
                public boolean onPreDraw() {
                    ...
                    switchAnimator.addListener(new AnimatorListenerAdapter() {
                        ...
                        @Override
                        public void onAnimationEnd(Animator animation) {
                            if (draggableChildren.get(originalPosition) != null) {
    ...
    /**
     * @param currentTop
     * @param views      bei denen überprüft werden soll, ob die gedraggte View sich innerhalb eienr üneregebenen View befindet
     * @return die View, in der sich die gedraggte View befindet
     * Optional.absent(), wenn keine View sich unterhalb der gedraggten View befindet.
     */
    private Optional<View> checkIsInnerView(int currentTop, View... views) {

        for (View view : views) {
            boolean isInner = checkIsInner(currentTop, view);
            if (isInner) {
    ...
    private boolean checkIsInner(int currentTop, View view) {

        if (view == null) {
            return false;
        }

        int topOfView = view.getTop();
        int heightOfView = view.getHeight();
        int oneThirdOfHeight = heightOfView / 3;
        int upperBound = topOfView + oneThirdOfHeight;
        int lowerBound = topOfView + oneThirdOfHeight * 2;
        return currentTop > upperBound && currentTop < lowerBound;
    }
    ...
    private class DragHandleOnTouchListener implements OnTouchListener, OnLongClickListener {
